...
namespace Cbi.FinancialSharedService.Database.Tests.StoredProcedures
{
    ...
    {
        private readonly string _paymentMessageHubConnection = DatabaseConnectionString.GetDatabaseConnectionString();
        ...
        public void SelectMessage_WhenCalledWithValidFilters_ReturnsMessagesThatMatchTheFilters()
        {
            ...
            using (var connection = new SqlConnection(_paymentMessageHubConnection))
            {
                firstRecordid = _dapperWrapper.Query<int>(connection,
                    "exec internal.UpsertMessage @Id, @Identifier, @EndPoint, @StatusId, @Attachment, @Body", new
                    {
                        Id = 0,
            ...
            Assert.AreEqual(secondRecordId, message.Id);
            Assert.AreEqual("IECBIBRS201911180000000000000002", message.Identifier);
